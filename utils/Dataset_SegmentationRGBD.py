...
class Dataset_SegmentationRGBD(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.width != -1 and self.height != -1:
            rgb_img = cv2.resize(rgb_img, (self.width, self.height), interpolation=cv2.INTER_NEAREST)  # .resize(rgb_img, (self.width, self.height), interpolation=cv2.INTER_NEAREST)
...
if __name__ == "__main__":
    ...
    dataset = Dataset_SegmentationRGBD(rovis_databases=database_test,
                                       width=320,
                                       height=320,
                                       transforms=None,
                                       device='cpu')
    ...
    max_imgs_to_disp = 5
    num_imgs_to_disp = min(max_imgs_to_disp, 16)
    ...
    for d in dataloader:
        ...
        for idx in range(to_be_shown):
            ...
            # rgb_orig /= 255.0
            #
            # semseg_out = decode_semseg(semseg, colormap)
        ...
        # cv2.waitKey(0)
            ...
            # cv2.imshow('rgb', np.swapaxes(rgb_orig, axis1=2, axis2=0))
            # cv2.waitKey(0)