...
class ResnetForHypernetV2(nn.Module):
    def __init__(self, in_channels: int = 3, num_classes: int = 10, type: str = 'resnet18', cached_layers: bool = True):
        super(ResnetForHypernetV2, self).__init__()
    ...
    def forward(self, x: torch.Tensor, embeddings_list: nn.ModuleList, hypernet: nn.Module, weights: list):
        if self.training:
            return self.forward_training(x, embeddings_list, hypernet)
        else:
            return self.forward_evaluating(x, weights)
    ...
    def forward_training(self, x: torch.Tensor, embeddings_list: nn.ModuleList, hypernet: nn.Module):
        if self.cached_layers:
            ...
            weights = [embeddings_list[emb_idx](hypernet) for emb_idx, _ in enumerate(embeddings_list)]
            for block_idx, _ in enumerate(self.resnet_blocks):
                x = self.resnet_blocks[block_idx](x, weights[4 * block_idx: 4 * block_idx + 4])
                cached_layers[layer_id] = x
                layer_id += 1
