...
from utils.Model_RGBDNet_Hypernet import Model_RGBDNet_Hypernet
...
def iou_coef(label, pred, smooth: int = 1):
    label = label.detach().cpu().numpy()
    pred = pred.detach().cpu().numpy()
    pred = np.argmax(pred, axis=1)

    intersection = np.count_nonzero(label == pred)

    union = 0
    for i in range(NUM_CLASSES):
        union += np.count_nonzero(label == i) + np.count_nonzero(pred == i)
    union -= intersection
...
train_dataset = Dataset_SegmentationRGBD(rovis_databases=database,
...
for epoch in range(epochs):
    ...
    with torch.no_grad():
        for batch_idx, batch_data in enumerate(train_dataloader):
            ...
            validation_IoU += iou_coef(labels, outputs)

            for i in range(outputs.shape[0]):
                semseg_output = outputs[i].detach().cpu().numpy()
                img_rgb_orig = cv2.resize(batch_data['original_img'][i].detach().cpu().numpy(),
        ...
        print('Validation loss: {}; validation IoU: {}%'.format(validation_loss, validation_IoU * 100.))
